<template>
  <div style="padding-left:10px;">
    <div>
      <h2 style="margin-top:15px;margin-bottom:13px;">学习态度记录-{{studentName}}</h2>
    </div>
    <div>
      <div>
        <Row>
...
export default {
  ...
  methods: {
    ...
    querySportsClassStudentHis() {
      var vm = this
      this.$axios.get('/v1/datafiles/queryHisSportsClassStudent?studentId='+vm.studentId+'&schoolType='+sessionStorage.getItem('schoolType')+'&yearSemester='+vm.yearSemester).then(res => {
        vm.loading=false;
  ...
  mounted() {
    this.studentId = sessionStorage.getItem('hisStudyLearningStudent-studentId');
    this.yearSemester = sessionStorage.getItem('hisStudyLearningStudent-yearSemester');
    this.studentName = sessionStorage.getItem('hisStudyLearningStudent-studentName');
