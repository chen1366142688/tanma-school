        ...
        <Card>
          <div class="top-search">
            ...
            <div class="right-btns">
              ...
              <Button type="success" v-if="semesterStatus == '1'" @click="inputStudentTest">体测数据导入</Button>
              <Button v-if="semesterStatus != '1'" disabled type="success">体测数据导入</Button>
              <Button class="add-button" v-on:click="exportSchoolTestData" type="success">导出EXCEL</Button>
            </div>
          </div>
        </Card>
      </div>
...
export default {
                ...
                return h("div", [
                  h(
                    "Button",
                    {
                      props: {
                        type: "success",
                        disabled: this.semesterStatus == "0" ? true : false,
                      },
                      style: {
                        marginRight: "5px",
                      },
  ...
  methods: {
    ...
    getStudentTestResultList() {
      this.$axios
        .get(
          "/v1/testdata/getStudentTestResultList?schoolId=" +
            this.schoolId +
            "&gender=" +
    ...
    getStudentTestResultListCount() {
      this.$axios
        .get(
          "/v1/testdata/getStudentTestResultListCount?schoolId=" +
            this.schoolId +
            "&gender=" +
            (this.gender == "999" ? "" : this.gender) +
            "&registerCode=" +
  ...
  mounted() {
    ...
    if (sessionStorage.getItem("queryStudentList-grade")) {
      this.grade = Number(sessionStorage.getItem("queryStudentList-grade"));
    } else {
      vm.grade = vm.schoolGradeList[0].grade;
    }

    if (sessionStorage.getItem("queryStudentList-studentClass")) {
      vm.studentClass = isNaN(
        Number(sessionStorage.getItem("queryStudentList-studentClass"))
      )
        ? ""
        : Number(sessionStorage.getItem("queryStudentList-studentClass"));
    } else {
    ...
    if (sessionStorage.getItem("queryStudentList-gender")) {
      this.gender = Number(sessionStorage.getItem("queryStudentList-gender"));
    } else {
      this.gender = 999;
    }

    if (sessionStorage.getItem("queryStudentList-exemptStatus")) {
      this.exemptStatus = Number(sessionStorage.getItem("queryStudentList-exemptStatus"));
    } else {
      this.exemptStatus = 999;
    }

    if (sessionStorage.getItem("queryStudentList-registerCode")) {
      this.registerCode = sessionStorage.getItem(
        "queryStudentList-registerCode"
      );
    }

    if (sessionStorage.getItem("queryStudentList-studentName")) {
      this.studentName = sessionStorage.getItem("queryStudentList-studentName");
    }

    if (sessionStorage.getItem("queryStudentList-pageNum")) {
      this.pageNum = Number(sessionStorage.getItem("queryStudentList-pageNum"));
    }

    if (sessionStorage.getItem("queryStudentList-pageSize")) {
      this.pageSize = Number(
        sessionStorage.getItem("queryStudentList-pageSize")
      );
    }

    if (sessionStorage.getItem("queryStudentList-scoreLevel")) {
      this.scoreLevel = sessionStorage.getItem("queryStudentList-scoreLevel");
    } else {
      this.scoreLevel = "999";
    }
