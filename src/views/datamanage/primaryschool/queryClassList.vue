...
export default {
  ...
  methods: {
    ...
    toDetail(item) {
      ...
      "studentManage-schoolTestData-classTestResult-activityId",item.activityId
    );
    sessionStorage.setItem(
      "studentManage-schoolTestData-classTestResult-classId",item.classId
    );
    sessionStorage.setItem(
      "studentManage-schoolTestData-classTestResult-gradeName",item.studentClass+'班'
    );
    sessionStorage.setItem(
      "studentManage-schoolTestData-classTestResult-startSchool",item.startSchool
    );
    sessionStorage.setItem(
      "studentManage-schoolTestData-classTestResult-schoolYear",item.schoolYear
    );
      this.$router.push({ name: "classTestResultThis" });
    },
    querySearch(){
      let vm = this;
      this.pageNum = 1;
    ...
    exportSchoolTestData(){
      ...
      let url =
        ...
        '/v1/testdataexport/exportThisYearClassData?grade='+(vm.grade==99?'':vm.grade)+'&schoolId=1&classId=' +(vm.classId==99?'':vm.classId)+
        '&token=' +
    ...
    search(){
      ...
      this.$axios
        .get(
          `/v1/testdata/queryThisYearActivityGradeList?grade=${vm.grade==99?'':vm.grade}&schoolId=${vm.schoolId}&type=2`
        )
        .then(res => {
          if (res.data.code == 10000) {
            ...
            this.$axios
              .get(
                `/v1/testdata/queryThisYearActivityClassList?grade=${vm.grade==99?'':vm.grade}&schoolId=${vm.schoolId}&classId=${vm.classId==99?'':vm.classId}`
              )
              .then(res => {
                if (res.data.code == 10000) {
