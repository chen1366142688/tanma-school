<template>
  <div style="padding-left:10px;">
    <div>
      ...
      >{{schoolYear}}学年体测-{{startSchool}}级 - {{gradeName}}</h2>
    </div>
    <Card>
      <div class="top-search">
        <div class="left-input">
          ...
          <div style="margin:5px 10px;">
            <span>完成状态：</span>
            <Select v-model="complete" style="width:120px;">
              <Option
                v-for="item in completeType"
                :value="item.typeId"
                :key="item.typeId"
              >{{ item.typeName }}</Option>
            </Select>
        ...
        <div class="right-btns">
          <Button class="add-button" v-on:click="getTeacherList()" type="success">查询</Button>
          <Button
            class="add-button"
...
export default {
      ...
      primaryColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            } else if (params.row.totalScoreLevel == "6") {
              return h("div", [
                h("span", params.row.totalScore != null ? 80 + "分" : ""),
              ]);
            } else {
              return h("div", [
                h(
  ...
  methods: {
    ...
    getClassTotal(grade) {
      this.$axios
        .get(
          "/v1/testdata/queryThisYearActivityClassList?grade=" +
            grade +
            "&schoolId=" +
            this.schoolId +
            "&classId=" +
        ...
        .then((res) => {
          if (res.data.code == 10000) {
            ...
            this.noTestCount =
              overViewData[0].needJoinTestCount - overViewData[0].overTestCount;
          }
        });
    },
    getYearSemester() {
    ...
    getTeacherList() {
      ...
      vm.loading = true;
      this.complete =
        this.complete == "1" ? "1" : this.complete == "2" ? "2" : "";
      this.$axios
        .get(
