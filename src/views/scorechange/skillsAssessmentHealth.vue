...
<template>
  <div style="padding-left:10px;">
    <div>
      <h2 style="margin-top:15px;margin-bottom:13px;">健康知识分析</h2>
    </div>
    <div>
        ...
        return {
            ...
            resultColumns: [
              ...
              },
              {
                ...
                minWidth:100,
                render: (h, params) => {
                  return h('div', [h('span', params.row.allStudentCount - params.row.healthGirlNotJoinStudentCount -params.row.healthBoyNotJoinStudentCount)])
                }
              },
              {
                title: '及格人数',
                ...
                minWidth:100,
                render: (h, params) => {
                  return h('div', [h('span', params.row.healthBoyPassStudentCount+params.row.healthGirlPassStudentCount)])
                }
              },
              {
                title: '及格男生人数',
                key: 'healthBoyPassStudentCount',
                align: 'center',
                minWidth:100,
              },
              {
                title: '及格女生人数',
                key: 'healthGirlPassStudentCount',
                align: 'center',
                minWidth:100,
              }
            ],
          ...
          this.$axios.get('/v1/examschool/queryGradeStudentShow?gradeId='+(vm.grade==999?'':vm.grade)+'&studentClass='+(vm.studentClass==999?'':vm.studentClass)).then(res => {
              if (res.data.code == 10000) {
                  ...
                  for(let val of this.resultData){
                    ...
                    this.completeCount += ((val.boyStudentCount - val.healthBoyNotOverStudentCount - val.healthBoyNotJoinStudentCount) + (val.girlStudentCount - val.healthGirlNotOverStudentCount-val.healthGirlNotJoinStudentCount));
                    
                    this.hangInTheAirCount += (val.healthBoyNotOverStudentCount + val.healthGirlNotOverStudentCount );
                    this.passCount += (val.healthBoyPassStudentCount + val.healthGirlPassStudentCount);
                    this.failCount += ((val.boyStudentCount - val.healthBoyPassStudentCount-val.healthBoyNotOverStudentCount-val.healthBoyNotJoinStudentCount) + (val.girlStudentCount - val.healthGirlPassStudentCount-val.healthGirlNotOverStudentCount-val.healthGirlNotJoinStudentCount));
                    this.needExamCount += val.allStudentCount - val.healthGirlNotJoinStudentCount - val.healthBoyNotJoinStudentCount;
                    gradeListData.push(gradeList[val.grade-1])
                    passListData.push(val.healthBoyPassStudentCount+val.healthGirlPassStudentCount );
                    failListData.push((val.boyStudentCount - val.healthBoyPassStudentCount-val.healthBoyNotOverStudentCount-val.healthBoyNotJoinStudentCount) + (val.girlStudentCount - val.healthGirlPassStudentCount-val.healthGirlNotOverStudentCount-val.healthGirlNotJoinStudentCount));
                  }
