...
export default {
  ...
  methods: {
    ...
    getAllGradeClassTestList() {
      ...
      this.$axios
        .get(
          '/v1/actionStamina/getAllGradeClassTestList?classTestType='+vm.classTestType+'&yearSemester='+vm.yearSemester
        )
        .then(res => {
          vm.loading = false;
          if (res.data.code == 10000) {
    ...
    baseSubmit() {
      ...
      let data = classList;
      this.$axios.get('/v1/actionStamina/querySchoolReportedStatus?dataType=2&yearSemester='+this.yearSemester).then(res => {
        if (res.data.code === 10000) {
    ...
    paramsCheck(){
        var vm=this;
        let yearSemester=Number(sessionStorage.getItem("actionStaminaHisConfige-actionStaminaConfigeChoose-yearSemester"));
        vm.yearSemester=yearSemester;
        if(vm.yearSemester%2==1){
            vm.yearName=(vm.yearSemester-9+"").substring(0,4)+"学年第二学期"
            vm.yearNameOther=(vm.yearSemester-9+"").substring(0,4)+"学年第一学期"
        }else{
            vm.yearName=(vm.yearSemester+"").substring(0,4)+"学年第一学期"
            vm.yearNameOther=(vm.yearSemester+"").substring(0,4)+"学年第二学期"
        }
        let gradeList=JSON.parse(sessionStorage.getItem("actionStaminaHisConfige-gradeList"));
