...
export default {
  ...
  methods: {
    ...
    exportData(taskId) {
      ...
      let url =this.$axios.defaults.baseURL +
        "/v1/datafiles/exportDataFilesStudentError?taskId=" +
        taskId +"&token=" +
    ...
    exportModel(taskId) {
      var vm = this;
      let url =this.$axios.defaults.baseURL +
        "/v1/datafiles/exportDataFilesStudentModel?token=" +
        sessionStorage.getItem("token");
      window.open(url);
    },
    viewIn() {
    ...
    getTaskList(schoolId) {
      ...
      this.$axios.post("/v1/datafiles/getDataFilesStudentTaskList", data).then(res => {
        vm.loading = false;
    ...
    countTaskListNum(schoolId) {
      ...
      this.$axios.post("/v1/datafiles/getDataFilesStudentTaskListNum", data).then(res => {
        if (res.data.code == 10000) {
  ...
  mounted() {
    ...
    vm.updateUrl =
      ...
      "/v1/datafiles/file/upload/addDataFilesStudentTaskInfo?schoolId=" +
